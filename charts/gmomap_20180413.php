...
<form name="live-map-gmo" method="POST" action="index.php?option=com_jumi&fileid=14" >

<div id="mainimage"><img src="images/backgrounds/GMO6.png" alt="" width="1300" />
...
// GMO Specific
function coord2screen($lat, $lon, $width) {
    //GMO
    $cx1 = 0.074;
    $cx2 = 0.933;
    $cy = 0.176;
...
function getkey($elem, $elements, $keys, $units, $projkey) {
  ...
  $calib = False;
  if ($calib) {
      $x1 = $width*0.074;  //x1 = 309891.947
      $x2 = $width*0.933;  //x2 = 310304.039
      $y = $width*0.176;
      ...
      list($x,$y) = coord2screen(310136.7,-34856.373,$width);
      ...
      list($x,$y) = coord2screen(310087.386,-34961.178,$width);
      echo '<div id="smallimage" STYLE="top:' . round($y-8,0) . 'px; left:' . round($x-8,0) . 'px;"><img src="images/backgrounds/bullet-green.png" Title="Test" /></div>';
  }
  ...
  if ($piercheck == 'checked') {
      ...
      while($row = mysqli_fetch_array($result)) {
          $pierid = $row[PierID];
          if (substr( $pierid, 0, 1 ) != 'P' and substr( $pierid, 0, 1 ) != 'T' and substr( $pierid, 0, 1 ) != 'Q') {
          ...
          while($lst = mysqli_fetch_row($tabs)) {
              ...
              for ($i = 0; $i <= sizeof($latlist); $i++) {
                ...
                if ($recentdata == 1) {
                 ...
                 list($x,$y) = coord2screen($latestdata[2],$latestdata[1],$width);
                 //echo '<p>' . $sensorid . ' : ' . $latestdata[2] . '</p>';
